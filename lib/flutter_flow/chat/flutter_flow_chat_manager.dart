...
class FFChatInfo {
  ...
  final List<GuestsRecord> groupMembers;

  GuestsRecord get currentUser => groupMembers
      .where((user) => user.reference == currentUserReference)
      .first;
  Map<String, GuestsRecord> get otherUsers => Map.fromEntries(
        groupMembers
            .where((user) => user.reference != currentUserReference)
            .map((user) => MapEntry(user.reference.id, user)),
      );
  List<GuestsRecord> get otherUsersList => otherUsers.values.toList();
  ...
  Stream<FFChatInfo> getChatInfo({
    ...
    return chatStream.asyncMap((chat) async {
      ...
      final groupMembers = await Future.wait(
        userRefs.map((ref) => GuestsRecord.getDocument(ref).first),
      );
